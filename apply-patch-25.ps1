<#
  Architect Hub -- patch 25: notifications actually clear once read,
  in both places they appear.
  Generated by Claude -- 2026-08-22

  Two separate notification displays in the app, both fixed the same
  way:

  1. The bell (top right, notification-bell.tsx): now fetches with
     unreadOnly=true (the API already supported this param, just
     wasn't being used), and clicking a notification removes it from
     the list immediately instead of just dimming it in place. "Mark
     all read" now clears the whole list at once. Simplified the
     rendering since every item shown is now always unread -- the old
     dimmed/greyed-out read styling was dead code once read items stop
     appearing at all.

  2. The dashboard's own Notifications card (separate component, pulls
     from the global store): now shows only unread notifications
     (reusing the `unread` filter already computed on that page)
     instead of the last 6 regardless of read status. Clicking one
     already did an optimistic read:true update in the store, which
     now makes it disappear immediately since the display filters by
     unread.
...
Write-Host "Applying Architect Hub patch 25 (notifications clear on read)..." -ForegroundColor Cyan

# --- [MOD] src/components/notifications/notification-bell.tsx ---
$content = @'
"use client";
...
'@
Write-PatchFile "src/components/notifications/notification-bell.tsx" $content
...
Write-Host ""
Write-Host "Patch 25 applied. No schema change -- just run:" -ForegroundColor Green
Write-Host "  npm run build"
Write-Host ""
Write-Host "To test: click a notification in the bell or the dashboard card -- it should vanish immediately, not just grey out." -ForegroundColor Green
