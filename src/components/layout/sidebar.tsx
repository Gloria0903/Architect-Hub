  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active
          ? "bg-blueprint/35 text-white font-medium"
          : "text-[#9AA7B2] hover:text-white hover:bg-white/5"
      )}
    >
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="px-2.5 pt-3.5 pb-1 text-[10px] tracking-wider text-[#5B6670]">
      {children}
    </div>
  );
}
...
export function Sidebar() {
  return (
    <aside className="bg-ink-sidebar w-[200px] shrink-0 p-3.5 flex flex-col gap-0.5 min-h-screen">
      ...
      <Link
        href="/projects"
        className="mt-auto bg-brick/[0.18] rounded-md p-2.5 hover:bg-brick/[0.28] transition-colors"
      >
        <div className="flex items-center gap-1.5 text-[#E8B7A2] text-[11px] font-semibold">
          <Repeat size={14} strokeWidth={1.8} />
          Take over project
        </div>
        <div className="text-[#8B96A0] text-[10.5px] mt-0.5 leading-tight">
          Instantly access any project&apos;s full history.
        </div>
      </Link>
    </aside>
  );
