<#
  Architect Hub -- patch 47 (updated): dark mode button fix PLUS
  /api/health was being blocked by middleware before it ever reached
  its own code.
  Generated by Claude -- 2026-08-28

  Same dark-mode button/backdrop fix as before (see prior notes: 31
  bg-ink occurrences switched to a new non-inverting bg-ink-solid
  token, bg-ink-sidebar correctly left alone).

  ADDED: /api/health was returning {"error":"Unauthorized"} when
  checked directly -- not from the health check's own logic (that
  route already has no auth check by design), but because middleware's
  session gate was blocking the request before it ever got there.
  Added it to the public-paths list, same pattern as /api/settings/firm
  and /api/cron/. This route exists specifically to work without a
  login session (load balancers, uptime monitors, manual diagnostics),
  so this was a real gap.
...
Write-Host "Applying Architect Hub patch 47 (dark mode + health check fix)..." -ForegroundColor Cyan
...
Write-Host "Push it, wait for the pipeline, then check https://architecthub.co.ke/api/health again --" -ForegroundColor Green
Write-Host "it should actually respond with real JSON now instead of Unauthorized." -ForegroundColor Green
