<#
  Architect Hub -- patch 21: payments simplified to one step, per your
  choice -- capped directly by contract budget, invoicing no longer
  required first.
  Generated by Claude -- 2026-08-22

  BACKEND (src/app/api/payments/route.ts): the core model changed from
  "Outstanding = Invoiced - Paid" (payments capped by what's invoiced)
  to "Outstanding = Budget - Paid" (payments capped directly by
  contract value). Removed the now-irrelevant "invoiced can't exceed
  budget" / "paid can't exceed invoiced" hierarchy checks -- both the
  pre-transaction check and the in-transaction race-condition-safe
  re-check were updated identically, so this can't drift between the
  two. Invoicing (patch 19) still exists and still works, it's just no
  longer a gate -- purely informational tracking now, exactly as you
  asked.

  FRONTEND (src/app/finance/page.tsx): the project dropdown, the live
  payment summary panel, and the submit-button validation all updated
  to the same budget-based cap. Also found and fixed a THIRD spot doing
  the old invoiced-based gating in the pre-submit client-side check
  (handleSubmit) that I'd missed on the first pass through this file --
  caught it by grepping for every remaining reference before calling
  this done.

  CHECKED BUT NOT CHANGED: the project detail page's own payment flow
  (projects/[id]/page.tsx) was ALREADY budget-based -- turns out that's
  exactly why you were seeing inconsistent behavcior depending on which
  page you paid from. Now both match.
...
Write-Host "Applying Architect Hub patch 21 (simplified payments)..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Patch 21 applied. No schema change -- just run:" -ForegroundColor Green
Write-Host "  npm run build"
Write-Host ""
Write-Host "To test: pick any project with a contract value and record a payment WITHOUT invoicing it first -- should now work, capped by contract value." -ForegroundColor Green
