<#
  Architect Hub -- patch 31: clients page fixes.
  Generated by Claude -- 2026-08-22

  Went through the whole page (create form, edit form, card layout,
  portal management) looking for genuine clutter or irrelevant
  features. The forms and edit modal were already clean -- no
  duplicated fields, no confusing labels. Found and fixed two real
  issues instead of guessing at cosmetic changes I can't verify:

  1. Address was collected in both the create AND edit forms but never
     displayed anywhere on the client card -- data going in with
     nowhere to come back out. Now shown on the card (only when
     present, with a map-pin icon matching the email/phone style).

  2. isAdmin and canManage were two separate variables with the exact
     same definition (role === "ADMIN") used interchangeably across
     the file -- consolidated to one.

  Deliberately did NOT touch the Client Portal section (enable/disable
  toggle, issue password) -- it's a real, relevant feature, and without
  a clearer signal that it's specifically what feels cluttered, hiding
  or collapsing it risks cutting something you actually want rather
  than removing something irrelevant.
...
Write-Host "Applying Architect Hub patch 31 (clients page)..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Patch 31 applied. Run:" -ForegroundColor Green
