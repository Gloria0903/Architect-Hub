# Scaling, Reliability, and Security â€” Where Architect Hub Actually Stands
...
- **Error boundaries** (`src/app/error.tsx`, `src/app/global-error.tsx`):
  ...
  button â€” nothing else changed about the underlying error, but a person
  hitting it isn't stuck.
- **Redis failures fail soft**: if `REDIS_URL` is unset or unreachable,
  the app logs one warning and keeps running â€” email/reminders just
...
- **API routes**: spot-checked across documents, logs, staff, and
  settings â€” consistent try/catch with a real error response, not an
...
- **Database**: Supabase Postgres. Supabase takes automatic daily backups
  on paid tiers, with point-in-time recovery on Pro and above â€” check
  your current plan in the Supabase dashboard (Settings â†’ Backups) and
...
- **Files**: S3 with versioning + encryption already correct in
  ...
  **bucket versioning** in the AWS console if you haven't â€” protects
...
- **Sessions are already fine.** JWT strategy, not database sessions â€” no
  sticky-session requirement, any instance can handle any request.
- **Database connections â€” fixed this pass.** `prisma/schema.prisma` now
  ...
  connection limit fast â€” this is one of the most common ways a "works
  fine in testing" app falls over the moment it's actually scaled.
- **In-memory rate limiting and the file-picker guard â€” known limitation,
  ...
  independently â€” someone could get `limit Ã— instance count` requests
  ...
  **I deliberately did not "fix" this by wiring the rate limiter to
  Redis** â€” `src/middleware.ts` (where the security-critical IP rate
  ...
  breaking rate-limiting on deploy, or crashing middleware outright â€”
  worse than the current limitation. The correct fix is confirming the
  middleware runtime first (or explicitly forcing
  `export const runtime = "nodejs"` if your Next.js version's stable â€”
...
- **Static assets**: whatever host you land on (Railway, per
  ...
  default â€” nothing extra needed here for a firm-sized user base.

## "Dynamic to changes going on" â€” notifications, logs, live updates
...
who did it** â€” confirmed this is already wired throughout `app-store.tsx`.
...
mechanism â€” Server-Sent Events or WebSockets â€” instead of polling. I
...
quick "more dynamic" fix â€” that directly fights the "many users" goal:
...
## Security â€” leak audit performed this pass
...
- Every API route has a real auth check (`auth()` for staff routes,
  `requireClientSession()` for the client portal â€” verified both exist
...
- `select` clauses on user-list endpoints are explicit allowlists, not
  broad object dumps â€” confirmed `mfaSecret`/`password` never appear in
