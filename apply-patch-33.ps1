<#
  Architect Hub -- patch 33: migrate from Supabase to Neon, using the
  Neon serverless driver adapter. This is what makes the app actually
  work on HostPinnacle's shared hosting -- confirmed by their own
  support that outbound Postgres (5432 AND 6543) is blocked entirely,
  policy-level, no ticket fixes it.
  Generated by Claude -- 2026-08-22

  CORE CHANGE: src/lib/prisma.ts now uses @prisma/adapter-neon instead
  of a raw TCP connection. The app queries the database over WSS
  (WebSocket Secure, port 443 -- same port as ordinary HTTPS), which
  gets through where a raw Postgres connection can't. Used
  unconditionally (not just for HostPinnacle) so every environment
  connects the same way.

  VERSION-PINNED CAREFULLY, not guessed: @prisma/adapter-neon 7.x
  (npm's default "latest") has no declared peer dependency and is
  built for Prisma 6.x/7.x -- installed and confirmed via real npm
  installs in a sandbox that this app's Prisma 5.22.0 core needs the
  EXACT matching 5.22.0 adapter release instead, paired with
  @neondatabase/serverless 0.10.4 (the peer range that version of the
  adapter actually declares). All three packages verified against
  their real, installed TypeScript type declarations before writing
  any code against them -- not assumed from documentation examples
  alone, which show a mix of version-specific APIs.

  IMPORTANT LIMITATION, confirmed via multiple official sources: 
  `prisma migrate deploy` ALWAYS uses a raw, direct TCP connection for
  schema operations, regardless of the driver adapter -- that only
  changes how the RUNNING APP queries data, not how migrations run.
  Practical result: migrations can never run from HostPinnacle's
  server itself (same blocked port), only from somewhere with
  unrestricted network access -- your own machine, or GitHub Actions.
  docs/DEPLOYMENT.md and the (still-disabled, needs secrets)
  deploy-hostpinnacle CI job both updated to reflect this correctly --
  the CI job now runs migrations as its own step directly on GitHub's
  runner, separate from the SSH-to-HostPinnacle step that only pulls
  code and restarts the app.

  Lint clean, YAML validated, 62/62 tests on your machine (same 3
  sandbox-only failures here, confirmed unrelated in every earlier
  patch).

  YOU NEED TO DO THIS FIRST, before applying: create a free Neon
  project (neon.tech), and from your OWN machine (not the HostPinnacle
  server) run:
    $env:DATABASE_URL = "<Neon pooled connection string>"
    $env:DIRECT_URL = "<Neon direct connection string>"
    npx prisma migrate deploy
  This is what actually creates your tables on the new database --
  nothing else in this patch does that for you.
...
Write-Host "Applying Architect Hub patch 33 (Neon migration)..." -ForegroundColor Cyan

# --- [MOD] prisma/schema.prisma ---
$content = @'
generator client {
  provider        = "prisma-client-js"
  // Required for the Neon serverless driver adapter (src/lib/prisma.ts)
  // -- lets Prisma talk to the database over HTTP/WebSocket instead of
  // a raw TCP connection. This is what makes the app work on hosting
  // that blocks outbound Postgres ports (5432/6543) but allows normal
  // HTTPS/WSS traffic on 443, like HostPinnacle's shared hosting.
  // GA (no longer "preview") from Prisma ORM 6.16.0 -- still a preview
  // feature on this app's current 5.22.0, which is why this flag is
  // here explicitly rather than just working by default.
...
'@
Write-PatchFile "src/lib/prisma.ts" $content
...
$content = @'
{
  ...
  "packages": {
    ...
    "node_modules/@aws-sdk/s3-request-presigner": {
      "version": "3.1106.0",
      "resolved": "https://registry.npmjs.org/@aws-sdk/s3-request-presigner/-/s3-request-presigner-3.1106.0.tgz",
      "integrity": "sha512-ZI5SCkyz8jB3Qr6NPSJ7R4A/PkniQvbD1DO8APwhMsubFcfwPSJMMUxrkv9k1E20ikRk7skpM9itZh+0wI9K/w==",
      "license": "Apache-2.0",
      "dependencies": {
        "@aws-sdk/core": "^3.977.6",
        "@aws-sdk/signature-v4-multi-region": "^3.996.43",
    ...
    "node_modules/@prisma/adapter-neon": {
      "version": "5.22.0",
      "resolved": "https://registry.npmjs.org/@prisma/adapter-neon/-/adapter-neon-5.22.0.tgz",
      "integrity": "sha512-yslwDPykmkVfrlyK3T9idewFcjINnqbkHZYJRxetoOZZSjBlJqqhsZxpJ/dEedUzeWaeDlWOgSEKjQKI46/A9w==",
      "license": "Apache-2.0",
      "dependencies": {
    ...
    "node_modules/@prisma/driver-adapter-utils": {
      "version": "5.22.0",
      "resolved": "https://registry.npmjs.org/@prisma/driver-adapter-utils/-/driver-adapter-utils-5.22.0.tgz",
      "integrity": "sha512-Y8msGZl9unmVflXoqdxTejv99UD02Gp4VoIvkyw+YxNUIj7nRz35O7yf5D87qNmTiPMGCS1WjUucG9ZuNq8+tw==",
      "license": "Apache-2.0",
      "dependencies": {
        "@prisma/debug": "5.22.0"
      }
    ...
    "node_modules/bullmq": {
      "version": "6.0.9",
      "resolved": "https://registry.npmjs.org/bullmq/-/bullmq-6.0.9.tgz",
      "integrity": "sha512-l7kNlirauwdZrqsViO6owHUAhSSN/aCDkrUwZQ2futIItlbJGAxgU+6IOQtTl5Re39GZJEPU6jelGuY1gQD28w==",
      "license": "MIT",
      "dependencies": {
        "cron-parser": "5.7.0",
        "msgpackr": "2.0.5",
...
'@
Write-PatchFile "package-lock.json" $content
...
jobs:
  ...
  #   if: github.ref == 'refs/heads/main' && github.event_name == 'push'
  #   steps:
  #     # Migrations run HERE, on GitHub's own runner -- NOT via the SSH
  #     # step below. prisma migrate deploy always needs a raw TCP
  #     # connection, which HostPinnacle's shared hosting blocks outbound
  #     # entirely (confirmed directly by their support) -- running it
  #     # over SSH on their server would fail the same way a direct
  #     # connection from the app used to. GitHub's runners have no such
  #     # restriction, so migrations happen here, then the SSH step only
  #     # pulls code and restarts the already-migrated app.
  #     - uses: actions/checkout@v4
  #     - uses: actions/setup-node@v4
  #       with:
  #         node-version: 22
...
Write-Host ""
Write-Host "Patch 33 applied. Run:" -ForegroundColor Green
Write-Host "  npm install"
Write-Host "  (this picks up the exact pinned adapter versions from package.json/package-lock.json)"
Write-Host "  npx prisma generate"
Write-Host "  npm run build"
Write-Host ""
Write-Host "REMINDER: if you haven't already run 'npx prisma migrate deploy' against Neon's" -ForegroundColor Yellow
Write-Host "DIRECT_URL from your own machine, do that BEFORE deploying -- see the top of this file." -ForegroundColor Yellow
