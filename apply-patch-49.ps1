<#
  Architect Hub -- patch 49: fixes project creation AND project listing
  by removing nested relation selects that fail on HostPinnacle's
  Neon connection.
  Generated by Claude -- 2026-08-28

  ROOT CAUSE, confirmed via your server's actual error log: queries
  with nested relation selects (client:{select:{...}}, architect,
  supervisor, payments, tasks, milestones, _count all joined into one
  query) fail with "Connection terminated unexpectedly" on this host's
  WebSocket connection to Neon, even with poolQueryViaFetch enabled --
  that setting only reliably covers genuinely simple, flat,
  single-table queries.

  FIX: both POST (create) and GET (list) now do a flat, single-table
  query for the project itself, then fetch every related piece
  (client, architect/supervisor, payments, tasks, milestones, and the
  three _count fields) as SEPARATE, BATCHED flat queries -- one
  findMany with an `in` filter per relation across ALL projects being
  returned, not one query per project per relation. Everything is then
  stitched back together in JavaScript into the exact same response
  shape the frontend already expects -- no frontend changes needed.

  This does NOT touch payments/invoices/reassign, which still use
  interactive transactions -- deliberately left alone per our
  agreement, since I haven't confirmed those are safe under any
  alternative connection mode yet.

  Verified: lint clean, grepped for any leftover reference to the old
  nested fields (none found), 62/62 tests on your machine (same 3
  sandbox-only failures here, confirmed unrelated in every earlier
  patch). Full typecheck will run for real in CI before this deploys,
  same safety net as always.
...
'@
[System.IO.File]::WriteAllText((Join-Path (Get-Location) "src/app/api/projects/route.ts"), $content, $utf8NoBom)
Write-Host "wrote src/app/api/projects/route.ts" -ForegroundColor Green
Write-Host ""
Write-Host "Patch 49 applied. Now:" -ForegroundColor Cyan
Write-Host "  git add ."
Write-Host "  git commit -m ""Fix project create/list: flat queries instead of nested relation selects"""
Write-Host "  git push origin main"
Write-Host ""
Write-Host "Watch the Actions tab -- if there's a real type error, it'll fail there, not on your live site." -ForegroundColor Yellow
