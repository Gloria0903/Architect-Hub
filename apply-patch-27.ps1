<#
  Architect Hub -- patch 27 (part 1 of this round): time-of-day greeting,
  and the real fix for "assignment emails don't work."
  Generated by Claude -- 2026-08-22

  1. GREETING: dashboard header now shows "Good morning/afternoon/
     evening/night, <FirstName>" based on the local hour, using the
     name already on the session -- above the existing "Admin
     Dashboard"/"My Dashboard" subtitle, which stayed as a smaller line
     underneath rather than being replaced.

  2. EMAIL NOTIFICATIONS -- root cause found and fixed, not just
     patched around: notification emails (project assignment, document
     uploads, client comments, payment updates) were ONLY ever enqueued
     to a BullMQ job for a worker process to send later. That worker
     (npm run worker) never runs at all on serverless hosting (Vercel)
     -- so on your current deployment, these emails were never
     delivered, no matter how correctly everything else was wired.

     Fixed at the shared dispatch() function all 6 notification types
     route through: emails now send DIRECTLY and synchronously via
     Resend's API within the same request that triggers them (e.g. the
     moment a project is created with an architect assigned), which
     works identically on Vercel or Railway -- no persistent worker
     required. Still also enqueues to the worker queue as a retry
     safety net if Redis is configured and the direct send fails, so a
     transient Resend hiccup still gets retried with backoff if a
     worker happens to be running.

     IMPORTANT DISTINCTION: this fixes emails triggered BY a user
     action (assignment, upload, comment, payment) -- exactly what you
     described. It does NOT fix deadline-approaching or missing-daily-
     report reminder emails, which are a different category: nothing
     triggers those from a request at all, something has to
     periodically CHECK for them, which genuinely still requires the
     worker process running somewhere (Railway). Those remain
     Railway-only until you migrate off Vercel.

  Lint clean on both files, 62/62 tests on your machine (same 3
...
Write-Host "Applying Architect Hub patch 27 part 1 (greeting + email fix)..." -ForegroundColor Cyan
...
'@
Write-PatchFile "src/lib/notifications.ts" $content

Write-Host ""
Write-Host "Patch applied. No schema change -- just run:" -ForegroundColor Green
Write-Host "  npm run build"
Write-Host ""
Write-Host "To test: make sure RESEND_API_KEY is set (locally and on Vercel), then create a project" -ForegroundColor Green
Write-Host "and assign an architect -- they should get an actual email now, even without the worker running." -ForegroundColor Green
